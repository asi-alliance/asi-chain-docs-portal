...
|----------|-------------|
| `BASE_FEE` | Base gas fee value |
| `VARIATION_RANGE` | Random variation range |
| `LABEL` | Fee label string |
| `TRANSFER` | Fee for transfer deploys |
| `DEPLOY` | Fee for generic deploys |
...
Generates a randomized gas fee by adding a random variation to the base fee. Returns a string formatted to 4 decimal places.

**Returns:** `string` — fee value (e.g., `"1.2345"`).
...
Returns the base gas fee as a number.

**Returns:** `number` — base fee amount.
...
Formats a fee value with the ASI label. If `fee` is omitted, generates a random fee.

**Parameters:**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `fee` | `string` | No | Fee value string. Generates a random fee if omitted. |

**Returns:** `string` — formatted fee with label (e.g., `"1.2345 ASI"`).
